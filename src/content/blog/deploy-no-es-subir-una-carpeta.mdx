...
La decisión ahí fue no tocar el código, que era la reacción instintiva. Se resolvió del lado del acceso. Lo que me dejó vale para cualquier proyecto que dependa de terceros: cuando delegás parte de tu sistema a un servicio, delegás también parte de tu diagnóstico.
...
Las tres formas de volver atrás actúan sobre lo que es tuyo: el código y los datos. Hay una parte que ninguna alcanza, y es lo que ya salió hacia otros sistemas.

En un checkout, por ejemplo, un pago puede aprobarse y la aplicación fallar antes de registrar la orden. Revertir el deploy no anula ese cobro. Restaurar un backup tampoco ayuda, y además puede borrar compras legítimas hechas después de esa copia.

<CheckoutGapBlock />

Para recuperar el sistema hay que reconciliar lo que registró la aplicación con lo que confirmó el proveedor de pagos, y resolver cada operación sin duplicar cobros ni perder órdenes válidas. Cómo se resuelve cada caso depende del flujo y de cuánto cuesta dejarlo inconsistente mientras tanto.

Volver atrás, entonces, no es una sola acción. Es decidir qué pasa con la versión del código, con los cambios en los datos y con las operaciones que ya llegaron a otros sistemas.

Y hay una condición previa que di por sentada: poder volver a ejecutar la versión anterior en un entorno compatible con producción. ¿Podrías levantar hoy la versión del mes pasado sin acordarte de ningún paso manual? De esa pregunta parte el próximo artículo, sobre qué puede hacer Docker más reproducible y qué queda fuera del contenedor.

---

Qué se puede deshacer, qué no y cuánto cuesta enterarse tarde es el tipo de decisión que documento en El Radar, con proyectos reales, por qué elegí lo que elegí y qué me costó. Si querés seguir la serie, suscribite acá abajo.
