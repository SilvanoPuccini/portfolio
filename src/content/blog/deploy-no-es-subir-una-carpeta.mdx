...
Volver a una versión anterior recupera el código, pero no necesariamente las operaciones que ya ocurrieron.

En un checkout, por ejemplo, un pago puede aprobarse y la aplicación fallar antes de registrar la orden. Revertir el deploy no anula ese cobro. Restaurar un backup completo también puede borrar compras legítimas hechas después de esa copia.

Para recuperar el sistema hay que reconciliar lo que registró la aplicación con lo que confirmó el proveedor de pagos, y resolver cada operación sin duplicar cobros ni perder órdenes válidas. Esa decisión depende del flujo y del costo que tendría dejarlo en un estado inconsistente.

Por eso, volver atrás no es una sola acción: hay que considerar la versión del código, los cambios en los datos y las operaciones que ya llegaron a otros sistemas.

También importa poder ejecutar la versión anterior en un entorno compatible con producción. El próximo artículo parte de esa pregunta y revisa qué puede hacer Docker más reproducible, y qué responsabilidades quedan fuera del contenedor.

---

En El Radar documento decisiones técnicas de proyectos reales, por qué las tomé y qué costo tuvieron. Si querés seguir la serie, podés suscribirte acá abajo.
