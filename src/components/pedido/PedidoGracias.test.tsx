...
describe('PedidoGracias', () => {
  it('agradece por su nombre y nombra lo que compró', () => {
    render(<PedidoGracias pedidoId="p1" nombre="Force Corp" paquete="Landing" pasos={pasos}
      factura={null} contacto="silvano@ejemplo.com" whatsapp="https://wa.me/1?text=x" locale="es" />);
  ...
  it('muestra el camino con la fecha de entrega', () => {
    render(<PedidoGracias pedidoId="p1" nombre="Ana" paquete="Landing" pasos={pasos}
      factura={null} contacto={null} whatsapp="https://wa.me/1?text=x" locale="es" />);
  ...
  it('firma con la letra de la marca', () => {
    render(<PedidoGracias pedidoId="p1" nombre="Ana" paquete="Landing" pasos={pasos}
      factura={null} contacto={null} whatsapp="https://wa.me/1?text=x" locale="es" />);

    expect(screen.getByText('SilvanoPuccini.dev')).toHaveStyle({ fontFamily: 'var(--font-firma), cursive' });
  });

  it('deja a mano el contrato y un canal para escribir', () => {
    render(<PedidoGracias pedidoId="p1" nombre="Ana" paquete="Landing" pasos={pasos}
      factura="0001-00000042" contacto="silvano@ejemplo.com" whatsapp="https://wa.me/1?text=x" locale="es" />);

    expect(screen.getByRole('link', { name: /contrato firmado/i })).toHaveAttribute('href', '/api/pedido/p1/contrato-firmado');
    expect(screen.getByRole('link', { name: /whatsapp/i })).toHaveAttribute('href', 'https://wa.me/1?text=x');
    expect(screen.getByRole('link', { name: /por correo/i })).toHaveAttribute('href', 'mailto:silvano@ejemplo.com');
