...
const BLOGS: PostPublicationListItem[] = [
  blog({ post_slug: 'post-uno', raw_title: 'El primer post', scheduled_at: '2026-09-06T11:00:00.000Z' }),
  blog({ post_slug: 'post-dos', raw_title: 'El segundo post', scheduled_at: '2026-09-13T11:00:00.000Z' }),
  blog({ post_slug: 'post-tres', raw_title: 'El tercer post', scheduled_at: '2026-09-20T11:00:00.000Z' }),
];

describe('orderedPosts', () => {
  it('numerates posts by chronological order', () => {
    const posts = orderedPosts(BLOGS);
    expect(posts.map((p) => [p.post_slug, p.number])).toEqual([
      ['post-uno', 1], ['post-dos', 2], ['post-tres', 3],
    ]);
  });
