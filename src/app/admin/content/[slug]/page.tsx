...
export default async function LinkedInDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!data) notFound();
  return <div style={{ maxWidth: 820, margin: '0 auto' }}><Link href="/admin/content" style={{ color: c.textDim, fontSize: 12 }}>← Biblioteca de LinkedIn</Link><h1 style={{ fontSize: 24, margin: '16px 0' }}>{data.title}</h1><LinkedInEditor item={data} /></div>;
}
